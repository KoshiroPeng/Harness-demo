...
- [deploy/release](.)：按单个微服务模块进行 SSH 发布、回滚和远端主机初始化。
- [deploy/compose](../compose)：按整套微服务进行 `docker-compose` 编排部署，更适合新环境搭建、整体验证和准生产演练。

如果需要先了解 workflow 职责分工，再回来看脚本细节，优先阅读 [.github/README.md](../../.github/README.md)。

真实后端源码入口是 [server](../../server)。当前发布主线按若依微服务模块组织制品、脚本和远端部署参数。

截至 2026-06-09，和发布前置验证直接相关的本地基线已经补齐：

- [server](../../server) 下 `mvn -B -DskipTests package` 已通过。
- [web](../../web) 下 `npm.cmd install` 与 `npm.cmd run build:prod` 已通过。
- 当前应把风险识别重点放在“远端部署参数是否与目标服务一致”。
...
- [../compose/README.md](../compose/README.md)：整套微服务 `docker-compose` 部署入口。
...
- `server-release.yml` 中的 `target_module` 是否与远端服务实际对应，不要把模块路径、Jar 文件名和 systemd 服务名混成一个概念。
...
## 与 docker-compose 部署的分工

如果当前发布目标是“替换单个服务版本”，优先沿用本目录的 SSH 发布方案。

如果当前发布目标是下面任一场景，更建议优先看 [deploy/compose/README.md](../compose/README.md)：

- 首次搭建完整环境
- 一次性联动多个微服务
- 希望把 Nacos、MySQL、Redis、网关、前端一起编排起来
- 需要更贴近真实运行拓扑地做整体验证

## 发布治理原则

- 发布材料的目标是减少真实交付风险，并保持脚本、workflow、环境变量和制品路径一致。
